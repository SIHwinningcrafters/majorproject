  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 24 }}>
      {steps.map((label, i) => {
        ...
        return (
          <div key={n} style={{ display: "flex", alignItems: "center", gap: 6, flex: i < steps.length - 1 ? "none" : undefined }}>
            ...
            {i < steps.length - 1 && (
              <div style={{
                width: 32, height: 1, margin: "0 6px",
...
export default function ReportIncidentModal() {
  ...
  const [coords,      setCoords]      = useState({ lat: 22.7196, lng: 75.8577 });
  ...
  const next = () => {
    ...
    if (step === 2) { const e = validateStep2(); if (e) { setErr(e); return; } }
    setStep((s) => s + 1);
  };
...
const submit = async () => {
  ...
  try {
    await createIncident({
      ...
      severity,
      description,
      location: { label: locLabel, lat: coords.lat, lng: coords.lng },
      anonymous,
    });
    triggerRefresh(); // 👈 tells all hooks to refetch
    setSubmitted(true);
  } catch (error) {
    setErr(error.response?.data?.message || "Failed to submit. Please try again.");
  } finally {
    setLoading(false);
  }
};
    ...
    return (
      <Overlay onClose={closeReportModal}>
        <div style={{ ...styles.modal, textAlign: "center", padding: "48px 32px" }}>
          ...
          <button style={styles.submitBtn} onClick={closeReportModal}>Back to map</button>
        </div>
      </Overlay>
    );
  }
          ...
          <div style={styles.stepWrap}>
            ...
            <div style={styles.anonRow}>
              ...
              <button
                ...
                style={{ ...styles.toggle, background: anonymous ? "var(--red)" : "var(--dark5)" }}
              >
                <span style={{
                  ...styles.toggleKnob,
          ...
          <div style={styles.stepWrap}>
            ...
            <div style={styles.reviewCard}>
              ...
              <div style={styles.reviewRow}>
                ...
                <span style={{ ...styles.reviewVal, color: SEVERITIES.find((s) => s.id === severity)?.color, fontWeight: 600, textTransform: "capitalize" }}>
                  ● {severity}
                </span>
              </div>
              <div style={styles.reviewDivider} />
              ...
              <div style={styles.reviewRow}>
                ...
                <span style={{ ...styles.reviewVal, fontFamily: "monospace", fontSize: 11 }}>
                  {coords.lat.toFixed(4)}, {coords.lng.toFixed(4)}
                </span>
              </div>
              <div style={styles.reviewDivider} />
...
/* ── STYLES ── */
const styles = {
  modal:       { width: "100%", maxWidth: 520, background: "var(--dark3)", border: "1px solid var(--border2)", borderRadius: "var(--r-xl)", padding: "28px 28px 20px", position: "relative", boxShadow: "0 24px 80px rgba(0,0,0,0.7)", maxHeight: "90vh", overflowY: "auto" },
  closeBtn:    { position: "absolute", top: 16, right: 16, background: "none", border: "none", color: "var(--muted)", fontSize: 16, cursor: "pointer" },
  brand:       { fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 15, display: "flex", alignItems: "center", gap: 7, marginBottom: 16 },
  brandDot:    { width: 8, height: 8, background: "var(--red)", borderRadius: "50%", display: "inline-block", animation: "pulse-dot 2s ease-in-out infinite" },
  stepWrap:    { display: "flex", flexDirection: "column", gap: 0 },
  title:       { fontFamily: "var(--font-display)", fontSize: 22, fontWeight: 800, letterSpacing: "-0.3px", marginBottom: 6 },
  sub:         { fontSize: 13, color: "var(--muted)", marginBottom: 20, lineHeight: 1.5 },
  sectionLabel:{ fontSize: 12, fontWeight: 600, color: "var(--muted2)", letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 10 },
  catGrid:     { display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 },
  catBtn:      { display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 5, padding: "10px 6px", borderRadius: "var(--r-sm)", border: "1px solid var(--border)", background: "var(--dark4)", cursor: "pointer", transition: "all .2s", color: "var(--muted)" },
  catBtnActive:{ borderColor: "var(--red)", background: "rgba(232,64,42,0.1)", color: "var(--text)" },
  catIcon:     { fontSize: 20 },
  catLabel:    { fontSize: 10, fontWeight: 600, textAlign: "center" },
  sevRow:      { display: "flex", flexDirection: "column", gap: 8 },
  sevBtn:      { display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: "var(--r-sm)", border: "1px solid var(--border)", background: "var(--dark4)", cursor: "pointer", transition: "all .2s", color: "var(--text)", textAlign: "left" },
  textarea:    { width: "100%", background: "var(--dark4)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)", padding: "10px 14px", color: "var(--text)", fontSize: 13, fontFamily: "var(--font-body)", outline: "none", resize: "vertical", lineHeight: 1.6 },
  charCount:   { fontSize: 11, color: "var(--muted)", textAlign: "right", marginTop: 4 },
  input:       { width: "100%", background: "var(--dark4)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)", padding: "10px 14px", color: "var(--text)", fontSize: 13, fontFamily: "var(--font-body)", outline: "none", marginBottom: 16 },
  anonRow:     { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: "var(--dark4)", borderRadius: "var(--r-sm)", border: "1px solid var(--border)" },
  toggle:      { width: 44, height: 24, borderRadius: 12, border: "none", position: "relative", cursor: "pointer", transition: "background .2s", flexShrink: 0 },
  toggleKnob:  { position: "absolute", top: 2, width: 20, height: 20, borderRadius: "50%", background: "#fff", transition: "transform .2s" },
  reviewCard:  { background: "var(--dark4)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: "4px 16px", marginBottom: 16 },
  reviewRow:   { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0" },
  reviewLabel: { fontSize: 12, color: "var(--muted)", fontWeight: 500 },
  reviewVal:   { fontSize: 13, fontWeight: 500 },
  reviewDivider:{ height: 1, background: "var(--border)" },
  disclaimer:  { fontSize: 11, color: "var(--muted)", lineHeight: 1.6, marginBottom: 4 },
  error:       { background: "rgba(232,64,42,0.1)", border: "1px solid rgba(232,64,42,0.25)", borderRadius: "var(--r-sm)", padding: "9px 13px", fontSize: 13, color: "#f87262", marginTop: 12 },
  navRow:      { display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 20, paddingTop: 16, borderTop: "1px solid var(--border)" },
  backBtn:     { padding: "10px 20px", borderRadius: "var(--r-sm)", border: "1px solid var(--border)", background: "transparent", color: "var(--muted)", fontSize: 13, cursor: "pointer" },
  nextBtn:     { padding: "10px 28px", borderRadius: "var(--r-sm)", border: "none", background: "var(--red)", color: "#fff", fontSize: 14, fontWeight: 700, fontFamily: "var(--font-display)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "opacity .2s" },
  submitBtn:   { padding: "11px 32px", borderRadius: "var(--r-sm)", border: "none", background: "var(--red)", color: "#fff", fontSize: 14, fontWeight: 700, fontFamily: "var(--font-display)", cursor: "pointer" },
};